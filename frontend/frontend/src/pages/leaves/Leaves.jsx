...
import { Plus, CheckCircle, XCircle, ChevronDown, ChevronUp, FileText, Info, Download } from 'lucide-react';
...
export default function Leaves() {
  ...
  // ── Download CSV — generates client-side from available data ──────
  const downloadCSV = () => {
    setDownloading(true);
    try {
      const headers = ['Employee Name', 'Leave Type', 'From Date', 'To Date', 'Total Days', 'Status', 'Reason'];
      ...
      a.href = objectUrl;
      a.download = `leave-report-${MONTHS[reportMonth - 1]}-${reportYear}.csv`;
      ...
      <div className="bg-black px-6 lg:px-8 pt-6 pb-5">
        <div className="flex items-center justify-between flex-wrap gap-4 animate-fade-in-up">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {/* ── Download CSV — all authenticated users ── */}
            {isAuthenticated && (
              <div className="flex items-center gap-2">
                <select
                  value={reportMonth}
                  onChange={e => setReportMonth(Number(e.target.value))}
                  className="bg-[#1F2937] border border-[#374151] text-white text-sm rounded-xl px-3 py-2 outline-none"
                >
                  {MONTHS.map((m, i) => (
                ...
                <input
                  ...
                  min="2020" max="2100"
                  className="bg-[#1F2937] border border-[#374151] text-white text-sm rounded-xl px-3 py-2 w-24 outline-none"
                />
                <button
                  onClick={downloadCSV}
                  disabled={downloading}
                  className="flex items-center gap-2 bg-[#10B981] hover:bg-[#059669] disabled:opacity-50 text-white font-bold px-5 py-2.5 rounded-full text-sm shadow-lg shadow-[#10B981]/25 transition-all"
                >
                  <Download size={15} />
